...
""",
        ...
        dist_info_select = {
            cfg: target + "_dist_info"
            for cfg, target in select_spec.items()
            if cfg != "//conditions:default"
        }
        if len(dist_info_select) == 1:
            dist_info_select["//conditions:default"] = dist_info_select.values()[0]
...
alias(
    name = "dist_info",
    actual = select({dist_info_select},
...
""".format(
                name = package_name,
                dist_info_select = indent(pprint(dist_info_select), "      ").lstrip(),
